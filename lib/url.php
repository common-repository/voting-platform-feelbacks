...
function _vcf_curl_urlopen($url, $postdata, &$response, $file_name, $file_field) {
	...
	if (version_compare(PHP_VERSION, '7.0.0') >= 0) {
		...
		/* check number of elements in $dataExplode array  */
		if(count($dataExplode) >= 2){
			list($headers, $response['data']) = $dataExplode;
		}elseif(count($dataExplode) == 1){
			list($headers) = $dataExplode;
			$response['data'] = '';
		}
	}else{
		list($headers, $response['data']) = $dataExplode;
...
function _vcf_get_response_headers($headers, &$response) {
	...
	$headers = explode("\r\n", $headers);
	$headersExplode = explode(' ', $headers[0], 3);
	
	if (version_compare(PHP_VERSION, '7.0.0') >= 0) {
		if(count($headersExplode) >= 3){
			list($unused, $response['code'], $unused) = $headersExplode;
		}elseif(count($headersExplode) == 2){
			list($unused, $response['code']) = $headersExplode;
		}elseif(count($headersExplode) == 1){
			list($unused) = $headersExplode;
			$response['code'] = '';
		}
	}else{
		list($unused, $response['code'], $unused) = $headersExplode;
